...
##########################
    ...
    async def cmd_skip(self, player, channel, author, message, permissions, voice_channel):
        ...
        if player.current_entry.meta.get('author', False) == None or permissions.instaskip or author == player.current_entry.meta.get('author', None):
            player.skip()
            return Response(
        ...
        #if not (m.deaf or m.self_deaf or m.id in [self.config.owner_id, self.user.id])
    ...
    async def cmd_queue(self, channel, player):
        ...
        for i, item in enumerate(player.playlist, 1):
            ...
            if item.meta.get('channel', False) and item.meta.get('author', False):
                ...
                nextline = '`{}.` **{}** {}'.format(i, item.title, songlen).strip()
...
####################################
    ...
    async def cmd_promote(self, player, channel, author, message, permissions, voice_channel, position=None):
        """
        Usage:
        ...
        if length < 2:
            ...
            return Response(reply_text, delete_after=30)
        """else:
            raise exceptions.PermissionsError("You don't have permission to promote songs!", expire_in=30)"""

    async def cmd_remove(self, player, author, position=1):
        """
        Usage:
        ...
        """
        try:
            if author.id == self.config.owner_id or author == list(player.playlist)[int(position)-1].meta['author']:
                if player.is_stopped:
                    raise exceptions.CommandError("Can't modify the queue! The player is not playing!", expire_in=20)
                ...
                raise exceptions.PermissionsError("You don't have permission to remove that!", expire_in=30)
        except IndexError:
            raise exceptions.CommandError("This is not a valid song number!", expire_in=20)
